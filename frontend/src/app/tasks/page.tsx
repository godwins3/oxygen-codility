  ...
  //             <button className="border p-2 rounded-lg flex items-center">+ add task</button>
            ...
            <div className="grid grid-cols-3 gap-4">
              {tasks.map((task) => (
                <div key={task.id} className="bg-white p-4 rounded-lg shadow-md">
                  <img src={task.img || "/assets/task.png"} alt={task.title} className="w-full h-40 object-cover rounded" />
                  <h3 className="text-lg font-semibold mt-3">{task.title}</h3>
                  <Progress value={task.progress} />
                  <p className="text-gray-600 mt-2">{task.time}</p>
                </div>
